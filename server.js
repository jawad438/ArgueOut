...
// ═══════════════════════════════════════════════════════════════════════
// LEGAL DOCS — Terms of Service / Privacy Policy, admin-editable via a rich
// text toolbar in the admin panel. Firestore doc legalDocs/{tos,privacy}
// holds { html, updatedAt }; these DEFAULT_* constants are only the
// day-one content (identical to what public/legal.html originally
// hardcoded) so the page and the admin editor both have real content to
// start from before anyone has ever saved an edit — once an admin saves,
// Firestore takes over as the source of truth and these constants are
// never touched again.
// ═══════════════════════════════════════════════════════════════════════
const DEFAULT_TOS_HTML = `
    <div class="legal-notice">
      These Terms of Service govern your use of ArgueOut, a live video debate platform. By creating an account or using the platform you agree to be bound by these terms in full.
    </div>
    ...
    <div class="legal-section">
      ...
      <p>The following content is never permitted anywhere on ArgueOut, including in debates, chat messages, polls posted on The Divide, and poll options, tags, or comments:</p>
      <ul>
        <li>Hate speech</li>
        <li>Child sexual abuse material (CSAM), or any content sexualizing minors</li>
        <li>Graphic or gratuitously violent content</li>
        <li>Sexually explicit content</li>
        <li>Any content that is illegal under the laws of your country or state of residence</li>
      </ul>
      <p>Content that violates these rules will be removed and may result in immediate account termination and, where required by law, a report to the relevant authorities. Any user can flag a poll for review using the report option on The Divide.</p>
    ...
    <div class="legal-section">
      ...
      <p>We may modify these Terms at any time. Material changes will be communicated via a notice on the website. Any change to these Terms is assumed to be agreed upon by all users as soon as it is posted — your continued use of ArgueOut after a change is published constitutes acceptance of the updated Terms, whether or not you have read them. If you do not agree with an updated version of these Terms, you must stop using ArgueOut.</p>
    </div>
...
const DEFAULT_PRIVACY_HTML = `
    <div class="legal-notice">
      This Privacy Policy explains how ArgueOut collects, uses, and protects your personal information. By creating an account you acknowledge this policy and agree to its terms.
    </div>
    ...
    <div class="legal-section">
      ...
      <p>If you use the <a href="/contact" class="cookie-banner-link">Contact</a> form, we additionally store the name, email address, subject, message, and any attached images/videos you submit, plus your IP address, so we can review and respond to your message.</p>
    </div>
    ...
    <div class="legal-section">
      ...
      <p>ArgueOut does not use cookies or local storage for advertising, and runs no analytics or tracking scripts today. Essential storage (keeping you signed in, connecting to a debate, remembering your consent choice) and optional functional storage (theme, saved debate state) are used, and you control the optional categories through a cookie consent banner and preference center. See the <a href="/cookies">Cookies page</a> for a full breakdown by category, including how a Do Not Track / Global Privacy Control signal is respected and how re-consent is requested if this policy changes or after 12 months.</p>
    </div>
...
// already uses) plus one io.emit so anyone currently online sees it
// immediately, instead of looping onlineUsers for a single target.
async function notifyAllUsers(message) {
  const usersSnap = await fstore.collection('users').select().get();
  const docs = usersSnap.docs;
  const BATCH_SIZE = 450;
  for (let i = 0; i < docs.length; i += BATCH_SIZE) {
    const batch = fstore.batch();
    docs.slice(i, i + BATCH_SIZE).forEach(userDoc => {
      const itemRef = fstore.collection('notifications').doc(userDoc.id).collection('items').doc();
      batch.set(itemRef, { type: 'admin', message, read: false, createdAt: admin.firestore.FieldValue.serverTimestamp() });
    });
    await batch.commit();
  }
...
app.get('/api/legal/:type', async (req, res) => {
  ...
  if (!LEGAL_DOC_TYPES[type]) return res.status(400).json({ error: 'Invalid document type' });
  try {
    const doc = await fstore.collection('legalDocs').doc(type).get();
    if (doc.exists && doc.data().html) {
      const d = doc.data();
      return res.json({ html: d.html, updatedAt: d.updatedAt ? d.updatedAt.toMillis() : null });
    }
    res.json({ html: LEGAL_DOC_TYPES[type], updatedAt: null });
  } catch { res.status(500).json({ error: 'Server error' }); }
});

// sanitize-html allowlist covers exactly what the admin toolbar can produce
// (bold/italic/underline/strike, legacy <font size> for the size picker,
// headings, paragraphs, lists, links) — this HTML renders unescaped to
// every visitor, so anything not on this list is stripped rather than
// trusted.
const LEGAL_SANITIZE_OPTIONS = {
  allowedTags: ['p', 'br', 'ul', 'ol', 'li', 'strong', 'b', 'em', 'i', 'u', 's', 'strike', 'font', 'h2', 'h3', 'a', 'div', 'span'],
  allowedAttributes: { a: ['href', 'target', 'rel'], font: ['size', 'color'], span: ['style'], div: ['class'] },
  allowedStyles: { span: { 'font-size': [/^\d+(?:px|rem|em)$/] } },
  allowedSchemes: ['http', 'https', 'mailto']
};

app.post('/api/admin/legal/:type', async (req, res) => {
  if (!await verifyAdminBearer(req)) return res.status(403).json({ error: 'Forbidden' });
  const type = req.params.type;
  if (!LEGAL_DOC_TYPES[type]) return res.status(400).json({ error: 'Invalid document type' });
  const html = sanitizeHtml(String(req.body?.html || ''), LEGAL_SANITIZE_OPTIONS);
  if (!html.trim()) return res.status(400).json({ error: 'Content is required' });
  try {
    await fstore.collection('legalDocs').doc(type).set({
      html, updatedAt: admin.firestore.FieldValue.serverTimestamp()
    });
    const label = type === 'tos' ? 'Terms of Service' : 'Privacy Policy';
    await notifyAllUsers(`Our ${label} has been updated. Continued use of ArgueOut means you agree to the changes — check the Legal page for details.`);
    res.json({ ok: true });
...
function pollDocToJson(doc) {
  ...
  return {
    ...
    trendingUntil: d.trendingUntil ? d.trendingUntil.toMillis() : null
  };
}
...
app.get('/api/polls/featured', async (req, res) => {
  try {
    ...
    const polls = sortPollsByInteraction(snap.docs.map(pollDocToJson))
      .filter(p => !(p.trendingUntil && p.trendingUntil > Date.now()))
      .slice(0, 3);
    res.json({ polls });
  } catch { res.status(500).json({ error: 'Server error' }); }
});
...
app.get('/api/polls', async (req, res) => {
  ...
  try {
    ...
    if (category === 'saved') {
      ...
      // Trending polls live in their own tab while trendingUntil is in the
      // future — they're deliberately absent from the plain "All" browse
      // until that window ends, so they don't feel duplicated across tabs.
      ...
      for (let i = 0; i < DIVIDE_SEARCH_MAX_SCAN_BATCHES && scanned.length < limit; i++) {
        ...
        if (category === 'trending') {
          zipped = zipped.filter(z => z.poll.trendingUntil && z.poll.trendingUntil > Date.now());
        } else if (category && category !== 'all') {
          zipped = zipped.filter(z => z.poll.category === category);
        }
        if (search) zipped = zipped.filter(z => (z.poll.question + ' ' + z.poll.tags.join(' ')).toLowerCase().includes(search));
        // Same "trending has its own tab" rule as the plain-browse branch
        // above — except an explicit search should still be able to find a
        // trending poll, since findability matters more than tab exclusivity
        // once the user is looking for something specific.
        if (category !== 'trending' && !search) {
          zipped = zipped.filter(z => !(z.poll.trendingUntil && z.poll.trendingUntil > Date.now()));
        }
        scanned.push(...zipped);
...
// Admins can close any poll; a poll's own creator can also close it early
// (results freeze, voting stops) — checked here rather than reusing
// verifyAdminBearer alone since a non-admin creator is a valid caller too.
app.post('/api/polls/:pollId/close', async (req, res) => {
  ...
  try {
    ...
    let allowed = !!(await verifyAdminBearer(req));
    if (!allowed) {
      const decoded = await getAuthUser(req);
      allowed = !!decoded && decoded.uid === doc.data().createdBy;
    }
...
// Admin-only: mark a poll as trending for N days, or clear it early.
app.post('/api/polls/:pollId/trending', async (req, res) => {
  if (!await verifyAdminBearer(req)) return res.status(403).json({ error: 'Forbidden' });
  const pollId = safeId(req.params.pollId);
  if (!pollId) return res.status(400).json({ error: 'Invalid poll' });
  ...
  try {
    ...
    await fstore.collection('polls').doc(pollId).update({ trendingUntil });
    res.json({ ok: true, trendingUntil: trendingUntil.toMillis() });
  } catch { res.status(500).json({ error: 'Server error' }); }
});

app.post('/api/polls/:pollId/trending/clear', async (req, res) => {
  if (!await verifyAdminBearer(req)) return res.status(403).json({ error: 'Forbidden' });
  const pollId = safeId(req.params.pollId);
  if (!pollId) return res.status(400).json({ error: 'Invalid poll' });
  try {
    await fstore.collection('polls').doc(pollId).update({ trendingUntil: admin.firestore.FieldValue.delete() });
    res.json({ ok: true });
  } catch { res.status(500).json({ error: 'Server error' }); }
});
